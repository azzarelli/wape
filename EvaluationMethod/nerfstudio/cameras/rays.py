"""
MODIFIED
    We have modified the RaySamples.getweights() fucntion and removed density from the delta density for opacity.
"""
import random
...
class RaySamples(TensorDataclass):
    ...
    def get_weights(self, densities: TensorType[..., "num_samples", 1]) -> TensorType[..., "num_samples", 1]:
        ...
        delta_density = densities #  self.deltas *
        alphas = 1 - torch.exp(-delta_density)

        transmittance = torch.cumsum(delta_density[..., :-1, :], dim=-2)
        # print(transmittance.shape, torch.zeros((*transmittance.shape[:1], 1, 1), device=densities.device).shape)
